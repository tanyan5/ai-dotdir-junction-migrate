...
| **防呆守卫** | 迁移前、清源前各检测一次目标应用是否仍在运行（进程名/命令行、SQLite `-wal`/`-shm` 未干净关闭、最新文件句柄被占），命中即拒绝执行——避免把正在写的数据库拷成"撕裂快照"。`-AllowRunningApp` 可强制放行。 |
...
- 迁移前**完全退出**目标应用（窗口 + 托盘图标 + 后台/serve 进程）。脚本会主动检测并拒绝：只有"没有任何进程占用、没有新鲜 WAL、最新文件无句柄锁"才放行——**"校验通过"不等于快照一致**，正在写的 SQLite 库拷过去会逻辑损坏（实测迁移 `~/.workbuddy` 时把 `sessions` 索引清空，任务列表变空，正文 jsonl 仍在）。
- **不要手动删除源目录**来"帮脚本一把"。清源失败说明进程还占着文件，正确做法是退出应用后**重跑同一条命令**（可续跑：会刷新拷贝、重试清源与建链接）。一个手误的 `Remove-Item` 路径就是真实数据。
- 想迁移**你自己正在用的那个应用**的数据目录（例如在 WorkBuddy 里迁移 `~/.workbuddy`）是不可能的：守卫永远拦。退出它，用别的工具或纯终端跑。
